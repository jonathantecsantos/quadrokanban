...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div className="flex flex-col w-full min-w-[300px] max-w-sm md:max-w-none bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 shadow-xl">
      {/* Header da Coluna */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-100 text-sm font-['Outfit']">{column.title}</h3>
              <span
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={`flex-1 flex flex-col gap-3 min-h-[450px] p-1 rounded-xl transition-colors duration-200 ${
              snapshot.isDraggingOver ? 'bg-slate-800/40 border-2 border-dashed border-indigo-500/40' : ''
            }`}
          >
            ...
            {cards.length === 0 && !snapshot.isDraggingOver && (
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center border-2 border-dashed border-slate-800/60 rounded-xl">
                <div className="p-3 bg-slate-800/40 rounded-full text-slate-600 mb-2">
                ...
                <button
                  type="button"
                  onClick={() => onAddCard(column.id)}
                  className="mt-3 px-3 py-1.5 text-xs font-medium text-indigo-400 hover:text-indigo-300 hover:bg-indigo-950/40 border border-indigo-900/50 rounded-lg transition-colors"
                >
                  + Criar tarefa
                </button>
              </div>
